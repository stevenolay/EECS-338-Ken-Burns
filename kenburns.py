...
def fetchBio(param):
    ...
    officesHeld = generateOfficesHeld(pageContent)
    officesHeld.insert(0, "Offices Held")
    ...
    summarized.insert(2, officesHeld)
...
def generateEarlyWork(content):
    workRelatedString = ""
    keyWords = ['study','studied' ,'employ', 'work', 'career', 'taught', 'directed', 'elected', 'director', 'teacher', 'professor', 'lawyer', 'worker', 'employer', 'intern', 'internship']
    ...
    workRelatedString = removeTags(workRelatedString, ["\[Parent Node\]"])
    workRelatedString = removeDuplicates(workRelatedString)
    return [workRelatedString.strip()]

def generateOfficesHeld(content):
    relatedString = ""
    keyWords = ['ran', 'elected', 'served', 'office', 'seek', 'seeking', 'election', 'race', 'won', 'win', 'loss', 'held', 'hold', 'senator', 'senate']
    contentKeys = ['election', 'legislation', 'representatives', 'legislative', 'senate', 'senator', 'representative']
    sectionIndex = 0
    for section in content:
        #print section[0], re.match("\[Parent Node\]",section[0], re.IGNORECASE)
        chainOut = False;
        if re.match("\[Parent Node\]",section[0], re.IGNORECASE):
            for cont in contentKeys:
                for substr in section[0].split():
                    if re.match(cont,substr, re.IGNORECASE):
                        relatedString += naiveSearch(content, sectionIndex, keyWords)
                        chainOut = True
                        break
                if chainOut:
                    break
    ...
    relatedString = removeTags(relatedString, ["\[Parent Node\]"])
    relatedString = removeDuplicates(relatedString)
    return [relatedString.strip()]
...
def removeTags(content, tags):
    newContent = content
    for tag in tags:
        m = re.search(tag, newContent, re.IGNORECASE)
        while m:        
            newContent = newContent[:m.start()] + newContent[m.end():]
            m = re.search(tag, newContent, re.IGNORECASE)
    return newContent 

def removeDuplicates(content):
    retContent = split_into_sentences(content)
    unique = []
    [unique.append(item) for item in retContent if item not in unique]

    return " ".join(unique)

# fetchBio("obama")
# fetchBio("bill clinton")
# fetchBio("hillary clinton")
# fetchBio("tim kaine")
# fetchBio("mike pence")
